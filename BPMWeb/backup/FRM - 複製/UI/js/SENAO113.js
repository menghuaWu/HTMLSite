...
function frmUIlLoad() {
  ...
  let updateDB = false;
  let subMsg = getMsgInfo(invokeURL, formId, "002", locale);
  if (
...
function senao113m037_process() {
  ...
  if (senao113m037.val()) {
    ...
    if (!$.isEmptyObject(userInfo)) {
      ...
      //alert("該負責業務查無對應所屬部門!");
      alert(getMsgInfo(invokeURL, formId, "037", locale));
    }
  }
  return true;
}
...
function senao113m010_process() {
  ...
  if (Object.keys(customerInfo).length !== 0) {
    ...
    alert(getMsgInfo(invokeURL, formId, "039", locale));
    senao113m010_reset();
  }
  return true;
}
  ...
  if ((inventoryItemStatusCode == "Active" || inventoryItemStatusCode == "C" ||
    ...
    if (isNotCheck_Credit_Cost.val() == "Y") {
      if (inventoryItemStatusCode != "C") {
        ...
        alert(getMsgInfo(invokeURL, formId, "041", locale));
        gsenao113d004_reset();
      }
      item_process();
    } else {
      ...
      if (itemNoForRBU.search(itemNo) > -1) {
        ...
        alert(getMsgInfo(invokeURL, formId, "042", locale));
        gsenao113d004_reset();
      }
      item_process();
    }

...
function item_process() {
  ...
  if (orderType.substr(0, 3) == "代收付") {
    if ("W/O#".search(itemNo) > -1 && explanation == "") {
      ...
      alert(getMsgInfo(invokeURL, frmType, "007", locale));
    } else if ("PVT".search(itemNo) > -1 && explanation == "") {
      //alert("[說明]欄位請填入領料單!!");
      alert(getMsgInfo(invokeURL, frmType, "009", locale));
    } else if ("Material".search(itemNo) > -1 && explanation == "") {
      //20190618 Milla 資訊服務申請單#8467 會計怡儒請GIGI將代收付料號Idle Stock更名為Material
      //alert("[說明]欄位請填入簽呈!!");
      alert(getMsgInfo(invokeURL, frmType, "011", locale));
    } else if ("NRE".search(itemNo) > -1) {
      //alert("請確認訂單是否全額轉收入?");
      alert(getMsgInfo(invokeURL, frmType, "013", locale));
    } else if (explanation == "") {
      //alert("代收付時,請輸入[說明]!!");
      alert(getMsgInfo(invokeURL, frmType, "016", locale));
    }
  }
}
/**
...
function isReminderCustomer(pCustomerId) {
  ...
  let alertMsg = getMsgInfo(invokeURL, frmType, "040", locale);

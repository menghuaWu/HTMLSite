...
(function ($) {
    ...
    function fixedColForDBcolapi(colapi) {
        ...
        if (colSettings.cols[0] != undefined) {
            colSettings.cols.forEach(function (e) {
                ...
                colHidden = (e.HIDDEN == undefined ? colHidden : (e.HIDDEN == 1 ? true : false));
                ...
                editAble = (e.EDITABLE == undefined ? editAble : (e.EDITABLE == 1 ? true : false));
    ...
    function GetColCfg(webapi) {
       ...
       // console.log('webapi',webapi);
        $.ajax({
            type: "POST",
            contentType: "application/json; charset=utf-8",
            url: invokeURL + "Service_GetColCfg",
            dataType: "json",
            data: JSON.stringify({
                p: webapi
            }),
    ...
    var get_edit_options = function (field, webapi) {
        ...
        if (field != "") {
            ...
            if (queryresult.length > 0) {
                ...
                var IS_JSON = true;
                try {
                    if (typeof ($.parseJSON(vars)) == 'object') { //參數JSON直接由table中的TEXT欄位取得
                        varjson = vars;
                    } else {
                        IS_JSON = false;
                    }
                ...
                if (IS_JSON == false) { //參數JSON由上一層網頁中的元素名稱與元素值組成
                    if (window.parent.length != 0) {
                        ...
                        for (var i = 0; i < varsary.length; i++) {
                            if (varsary[i].substr(0, 1) == "@") {
                                ...
                                var celValue = $("#jqGrid").jqGrid('getCell', selRowId, keyname);
                                varjson[keyname] = celValue;
                            } else {
                                varjson[varsary[i]] = $("#" + varsary[i], parent.document).val();
                            }
            ...
            $.ajax({
                ...
                success: function (JSONdata) {
                    if (JSONdata.length > 0) {
                        if (JSONdata[0]["result"] == undefined) {
                            JSONdata.forEach(function (data, index) {
                                if (data["OPTIONTYPE"] == "dataInit") { // dataInit時執行value欄位的function
                                    ret[data["OPTIONTYPE"]] = eval(data["VALUE"]);
                                } else {
                                    if (data["VALUE"].indexOf(":") == 0) { // VALUE若為冒號(:)開頭的變數時

                                        var nodeName = $("#" + data["VALUE"].substr(1), parent.document).prop("nodeName"); //取得母頁面元素tagName
                                        if (nodeName == "DIV" || nodeName == "SPAN") {
                                            ret[data["OPTIONTYPE"]] = $("#" + data["VALUE"].substr(1), parent.document).text();
                                        } else {
                                            ret[data["OPTIONTYPE"]] = $("#" + data["VALUE"].substr(1), parent.document).val();
                                        }
            ...
            $.ajax({ //ajax start
                ...
                success: function (result) { //success start
                    ...
                    if (options.fixedColFDb) {
                        ...
                        if (!err) {
                            if (options.colAPI != '' && options.colAPI != undefined) {
    ...
    function setPager(options) {
        ...
        let prmAdd = { //add Options. save key parameter will keybind the Enter key to submit.
            ...
            serializeEditData: function (postData) {
                postData["station"] = chkGridEditable(getUrlParameters(options.addOption.addurl)["sCode"]);
        ...
        let prmEdit = { //Edit Options. save key parameter will keybind the Enter key to submit.      
            ...
            serializeEditData: function (postData) {
                ...
                Object.keys(rowData).forEach(function (item) {
                    ...
                    postData["station"] = chkGridEditable(getUrlParameters(options.editOption.editurl)["sCode"]);
                });
    ...
    function chkGridEditable(webapi) {
        ...
        $.ajax({
            type: "POST",
            contentType: "application/json; charset=utf-8",
            url: invokeURL + "Service_ChkListEditStation",
            dataType: "json",
            data: JSON.stringify({
                p: webapi
            }),

/**對API讀取Navbar設定
 *
 * @param {String} parent 上層元素
 * @param {String} apiUrl 呼叫API的路徑
 * @param {String} serviceType API的參數
 */
function getNavbarConfig(parent, apiUrl, serviceType) {
  $.ajax({
    type: "POST",
    // url: "https://apiportal.senao.com/MES/invoke9?sCode=" + serviceType,
    url: apiUrl + serviceType,
    data: "{}",
    dataType: "json",
    contentType: "application/json; charset=utf-8",
    success: function (navbarConfig) {
     // console.log('navbarConfig',navbarConfig);
      SetNavbar(parent, navbarConfig, {
        txtColor: "navbar-dark",
        bgColor: "bg-dark",
      });
      NavbarClick();
    },
    error: function (err) {
      console.log("navbar_test_err", err);
    },
  });
}
...
function SetNavbar(tag, json, navbarColor) {
  ...
  /*document.documentElement.style.setProperty('--bgColor', '#000000'); //改變CSS變數
	...
	console.log(value);
	*/
  navbarColor["txtColor"] =
    navbarColor["txtColor"] == undefined
      ? "navbar-dark"
      : navbarColor["txtColor"];
  navbarColor["bgColor"] =
    navbarColor["bgColor"] == undefined ? "bg-dark" : navbarColor["bgColor"];
  if (parent) {
    ...
    $("head").append(
      '<link rel="stylesheet" type="text/css" href="/GBLWeb/Module/css/navbar.css">'
    );
    ...
    nav.className =
      "navbar navbar-expand-md mainmenu navbar-hover " +
      navbarColor.txtColor +
      " " +
      navbarColor.bgColor;
...
function SetNavbarContent(tag, json) {
  ...
  $.each(json, function (index, element) {
    ...
    if ((element.PARENTS === null || element.PARENTS == "") && (element.CHILD === null || element.CHILD === "")) {
      //第一層
      li.className = "nav-item";
      a.className = "nav-link";

      li.appendChild(a);
    } else if ((element.PARENTS === null  || element.PARENTS =="") && (element.CHILD === "Y")) {
    ...
    } else if (element.PARENTS !== null && element.CHILD === "Y") {
